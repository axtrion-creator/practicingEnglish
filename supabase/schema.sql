...
-- Policies below are set for public read-only gameplay.
-- Anonymous users can read words, but cannot write any table data.
...
drop policy if exists words_delete_anon on public.words;
...
drop policy if exists progress_sessions_update_anon on public.progress_sessions;
drop policy if exists progress_sessions_delete_anon on public.progress_sessions;
...
drop policy if exists progress_word_stats_update_anon on public.progress_word_stats;
drop policy if exists progress_word_stats_delete_anon on public.progress_word_stats;
...
drop policy if exists user_stats_delete_anon on public.user_stats;
